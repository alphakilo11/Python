...
def read():
	...
	temperature = int(temperature) / 1000 # According to the datasheet the highest resolution is 2 ** -4 = 0.0625 °C.
	return temperature
		...
		while True:
			...
			if store_now or (now[2] == next_save_day and now[3] >= 2): #triggers once a day after 2 o'clock 
				...
				for i in delete_list:
  					temperaturelist = temperaturelist.replace(i[0], i[1])
				with open(filename, "w") as file:
					file.write(temperaturelist)
